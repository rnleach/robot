...
int main (void)
{
    ...
    while(1){
        ...
        volatile uint16_t cnt = Timer1::get_count();
        ...
        while(get_pin_value(Pins::D11_PWM) == PinValue::HIGH)
        {
            ...
            volatile uint16_t new_cnt = Timer1::get_count();
            if( new_cnt < cnt){
                rolled = 1;
                down_loops++;
        ...
        uint16_t curr_cnt = Timer1::get_count();
        uint16_t sub_ticks = (curr_cnt - cnt);
        if(curr_cnt < cnt) sub_ticks -= Timer1::get_max() - Timer1::get_top();
        uint16_t ticks = rolls * (Timer1::get_top() + 1) + sub_ticks;
        printf("Set high found at cnt = %u\n", cnt);
        printf("Looped on high = %u with down_loops = %u and up_loops = %u\n", loops, down_loops, up_loops);
        printf("Set low found! %u * %u + %u = %u\n", rolls, (Timer1::get_top() + 1), sub_ticks,ticks);
        // uint32_t us = ticks * (32* 1000000UL / F_CPU);
        uint32_t us = static_cast<uint32_t>(ticks * (static_cast<float>(Timer1::get_prescale()) * 1000000.0 / F_CPU));
        printf("ticks = %u\nprescale = %u\n", ticks,Timer1::get_prescale());
        printf("multiplier = %f\n", (static_cast<float>(Timer1::get_prescale()) * 1000000.0 / F_CPU));
        printf("us = %lu\n", us);
        printf("cm = %lu\n\n", us / 58);
        _delay_ms(2000);
    }
